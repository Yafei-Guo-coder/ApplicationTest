...
for block_id, row in tqdm(bed_df.iterrows(), total=len(bed_df), desc="Processing blocks"):
    ...
    # 预分配基因型数组（避免重复分配）- 这是关键优化
    geno_array = np.empty(n_samples, dtype=np.int32)
    
    # 批量读取该block所有变异的基因型（更高效且安全）
    print(f"  正在读取 {len(var_idx)} 个变异的基因型...")
    all_genotypes = []
    for j, global_var_idx in enumerate(var_idx):
        try:
            ...
            # 只保存我们需要的样本
            all_genotypes.append(geno_array[pheno_idx].copy())
        except Exception as e:
            print(f"\n  ⚠️  变异 {j} (全局{global_var_idx}) 读取失败: {e}")
            # 用missing值填充
            all_genotypes.append(np.full(len(pheno_idx), -9, dtype=np.int32))
    
    # 转为矩阵: (n_variants × n_samples_filtered)
    geno_matrix = np.array(all_genotypes)
    print(f"  基因型矩阵: {geno_matrix.shape}")
