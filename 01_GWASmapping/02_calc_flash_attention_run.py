...
@torch.no_grad()
def attn_column_sums_streaming(q, k, causal=False, block_cols=1024):
    """
...
def calc_attentions_streaming(seq: str, model, tokenizer, device, block_cols=1024) -> List[float]:
    """
    使用流式方法计算注意力分数
    """
    captured.clear()
    ...
    col_sums = attn_column_sums_streaming(q, k, causal=False, block_cols=block_cols)
    vec = col_sums[0]
...
def parse_arguments():
    ...
    parser.add_argument("--output_suffix", type=str, default="_streaming")
    parser.add_argument("--block_cols", type=int, default=1024, help="流式计算的块大小")
        ...
        for sample in tqdm(samples, desc="处理样本"):
            ...
            try:
                ...
                attn_scores = calc_attentions_streaming(
                    seq, model, tokenizer, device, block_cols=args.block_cols
                )
